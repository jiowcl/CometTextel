...
This requests a report; use `GsmModem.PollStatusReport()` to drain `+CDS`
URCs after a send with `requestStatusReport: true` (C ABI v3). Automatic
callback-style delivery tracking is not provided.
